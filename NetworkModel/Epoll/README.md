...
3) 엣지 트리거(ET): 상태 변화가 있을 때만 알려줌
동작원리<br>
조건이 거짓→참으로 바뀌는 순간(엣지)만 이벤트를 준다.<br>
예) 수신 버퍼가 0바이트였다가 100바이트가 들어온 순간 1번 알림<br>
그 뒤에 버퍼에 데이터가 남아 있어도, 상태 변화가 없으면 더 이상 안 알려줄 수 있다.<br>
즉, 한 번 깨워줬을 때 네가 “끝까지 처리”해야 한다.<br>
특징<br>
wakeup 횟수를 줄여 성능 이점 가능(고성능 서버가 ET를 많이 선택)<br>
대신 구현이 까다로움:<br>
반드시 non-blocking<br>
반드시 drain 패턴(EAGAIN까지 루프)<br>
안 지키면 “이벤트를 놓치고” 해당 연결이 영원히 멈춘 것처럼 보일 수 있음 (가장 흔한 버그)<br>

4) 가장 중요한 차이: “조금만 처리하고 나와도 되는가?”
상황	LT (Level)	ET (Edge)
